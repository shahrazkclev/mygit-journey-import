...
BEGIN
  ...
  IF unsubscribed_record IS NOT NULL THEN
    ...
    INSERT INTO public.contacts (
      ...
      first_name = COALESCE(unsubscribed_contacts.first_name, contacts.first_name),
      last_name = COALESCE(unsubscribed_contacts.last_name, contacts.last_name),
      tags = CASE 
        WHEN unsubscribed_contacts.tags IS NOT NULL AND array_length(unsubscribed_contacts.tags, 1) > 0 THEN 
          CASE 
            WHEN contacts.tags IS NULL THEN unsubscribed_contacts.tags
            ELSE ARRAY(SELECT DISTINCT unnest(contacts.tags || unsubscribed_contacts.tags))
          END
        ELSE contacts.tags
      END,
      status = 'subscribed'
    FROM public.unsubscribed_contacts
    WHERE unsubscribed_contacts.user_id = p_user_id AND unsubscribed_contacts.email = p_email;
