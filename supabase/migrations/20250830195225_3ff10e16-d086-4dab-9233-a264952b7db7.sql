-- Add multi-trigger support to tag_rules
ALTER TABLE public.tag_rules 
  ADD COLUMN trigger_tags text[] DEFAULT ARRAY[]::text[],
  ADD COLUMN trigger_match_type text DEFAULT 'any' CHECK (trigger_match_type IN ('any', 'all'));

-- Migrate existing single trigger_tag to trigger_tags array
UPDATE public.tag_rules 
SET trigger_tags = ARRAY[trigger_tag] 
WHERE trigger_tag IS NOT NULL AND trigger_tags IS NULL;

-- Update the trigger function for multi-trigger support
CREATE OR REPLACE FUNCTION public.apply_tag_rules_trigger()
RETURNS TRIGGER AS $$
DECLARE
  rule_record RECORD;
  updated_tags text[];
  has_changes boolean := false;
  trigger_matched boolean := false;
  match_count integer := 0;
  required_matches integer := 0;
  add_tags_length integer;
  remove_tags_length integer;
BEGIN
  -- Only process if tags actually changed
  IF OLD.tags IS NOT DISTINCT FROM NEW.tags THEN
    RETURN NEW;
  END IF;

  updated_tags := NEW.tags;
  
  -- Loop through all enabled tag rules for this user
  FOR rule_record IN 
    SELECT * FROM public.tag_rules 
    WHERE user_id = NEW.user_id AND enabled = true
  LOOP
    trigger_matched := false;
    match_count := 0;
    
    -- Check trigger tags
    IF rule_record.trigger_tags IS NOT NULL AND array_length(rule_record.trigger_tags, 1) > 0 THEN
      required_matches := CASE 
        WHEN rule_record.trigger_match_type = 'all' THEN array_length(rule_record.trigger_tags, 1)
        ELSE 1 -- 'any'
      END;
      
      -- Count matches between contact tags and trigger tags
      IF updated_tags IS NOT NULL THEN
        FOR i IN 1..array_length(rule_record.trigger_tags, 1) LOOP
          FOR j IN 1..array_length(updated_tags, 1) LOOP
            IF lower(trim(updated_tags[j])) = lower(trim(rule_record.trigger_tags[i])) THEN
              match_count := match_count + 1;
              EXIT; -- Exit inner loop once we find this trigger tag
            END IF;
          END LOOP;
        END LOOP;
      END IF;
      
      trigger_matched := match_count >= required_matches;
    END IF;
    
    IF trigger_matched THEN
      -- Add tags if specified
      add_tags_length := array_length(rule_record.add_tags, 1);
      IF add_tags_length IS NOT NULL AND add_tags_length > 0 THEN
        FOR i IN 1..add_tags_length LOOP
          IF updated_tags IS NULL THEN
            updated_tags := ARRAY[trim(rule_record.add_tags[i])];
            has_changes := true;
          ELSIF NOT (trim(rule_record.add_tags[i]) = ANY(updated_tags)) THEN
            updated_tags := array_append(updated_tags, trim(rule_record.add_tags[i]));
            has_changes := true;
          END IF;
        END LOOP;
      END IF;
      
      -- Remove tags if specified
      remove_tags_length := array_length(rule_record.remove_tags, 1);
      IF remove_tags_length IS NOT NULL AND remove_tags_length > 0 THEN
        FOR i IN 1..remove_tags_length LOOP
          updated_tags := array_remove(updated_tags, trim(rule_record.remove_tags[i]));
          has_changes := true;
        END LOOP;
      END IF;
    END IF;
  END LOOP;
  
  -- Update tags if changes were made
  IF has_changes THEN
    NEW.tags := updated_tags;
  END IF;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = '';