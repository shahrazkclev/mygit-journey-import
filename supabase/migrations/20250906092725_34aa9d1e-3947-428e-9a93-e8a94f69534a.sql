-- Update apply_tag_rules_pure function to handle replace_all_tags option
...
BEGIN
  ...
  FOR rule_record IN 
    ...
    ORDER BY created_at ASC  -- Process rules in order they were created
  LOOP
    match_count := 0;
    required_matches := 0;
    ...
    -- Check if this rule should replace all tags
    IF coalesce(rule_record.replace_all_tags, false) = true THEN
      -- Replace all tags with only the add_tags
      add_len := coalesce(array_length(rule_record.add_tags, 1), 0);
      IF add_len > 0 THEN
        updated_tags := ARRAY[]::text[];  -- Clear all existing tags
        FOR i IN 1..add_len LOOP
          IF rule_record.add_tags[i] IS NOT NULL AND length(trim(rule_record.add_tags[i])) > 0 THEN
      ...
      END IF;
    ELSE
      -- Normal behavior: add and remove tags
